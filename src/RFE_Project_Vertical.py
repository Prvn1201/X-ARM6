...
tgtrip_file_path = "/home/uf/.UFACTORY/projects/test/xarm6/python/nope/verticalgcode.txt"

# Function to move robot to a position
def move_robot(x, y, z, first_move=False, second_move=False):
    if arm.error_code == 0 and not params['quit']:
        # Apply rotation (90-degree around X-axis)
...
try:
    with open(tgtrip_file_path, 'r') as file:
        lines_batch = []
        line_count = 0
        with ThreadPoolExecutor(max_workers=3) as executor:  # Limit to 3 concurrent tasks
            for line in file:
...
# Release pin 5 when the program finishes
if not params['quit']:
    if arm.set_cgpio_digital(5, 0) != 0:
        params['quit'] = True

# Move the robot back to the initial position after the program finishes
if arm.error_code == 0 and not params['quit']:
    if arm.set_position(*[228.0, 0.0, 385.0, 0.0, -90.0, 180.0], speed=params['speed'], mvacc=params['acc'], radius=-1.0, wait=True) != 0:
        params['quit'] = True
